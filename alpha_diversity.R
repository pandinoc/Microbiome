...
setwd('/Users/paolaandino/Documents/Microbiome')
list.files()
...
meta<-read_q2metadata("MetadataS3.txt")
str(meta)
colnames(meta)[2] <- "treatment"
colnames(meta)[6] <- "donor"
str(meta)
...
faith_pd <-faith_pd[,-1]
colnames(faith_pd)[colnames(faith_pd) == "V1"] = "sample-id"
colnames(faith_pd)[colnames(faith_pd) == "V2"] = "V1"
...
colnames(meta)[colnames(meta) == "SampleID"] = "sample-id"
...
colnames(meta)[colnames(meta) == "V1"] = "faith_pd"
...
ggsave("output/evenness.png", evenness_boxplot, height = 3, width = 3)

ggplot(meta, aes(subject, pielou_evenness, color=subject)) + 
  geom_boxplot() + 
  facet_grid(~body.site.ord) +
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
...
kruskal.test(faith_pd ~ treatment, data=meta)
...
#Plot
boxplot(faith_pd ~ treatment, data=meta, ylab="Faith phylogenetic diversity")

# or with ggplot2

faith_pd <- ggplot(meta, aes(treatment, faith_pd)) + 
...
ggsave("output/4pd.png", faith_pd, height = 3, width = 3)

kruskal.test(faith_pd ~ donor, data=meta)
faith_pd <- ggplot(meta, aes(donor, faith_pd)) + 
  geom_boxplot(aes(color = donor)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  theme(legend.title = element_blank()) +
  labs(y="Faith Phylogenetic Diversity", x = "") 
ggsave("output/pd2.png", faith_pd, height = 3, width = 3)


kruskal.test(pielou_evenness ~ treatment, data=meta)
pielou_evenness <- ggplot(meta, aes(treatment, pielou_evenness)) + 
  geom_boxplot(aes(color = treatment)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  theme(legend.title = element_blank()) +
  labs(y="Peilou evenness", x = "") 
ggsave("output/4pe.png", pielou_evenness, height = 3, width = 3)

kruskal.test(pielou_evenness ~ donor, data=meta)
pielou_evenness <- ggplot(meta, aes(donor, pielou_evenness)) + 
  geom_boxplot(aes(color = donor)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  theme(legend.title = element_blank()) +
  labs(y="Peilou evenness", x = "") 
ggsave("output/pe2.png", pielou_evenness, height = 3, width = 3)

kruskal.test(observed_features ~ treatment, data=meta)
observed_features <- ggplot(meta, aes(treatment, observed_features)) + 
  geom_boxplot(aes(color = treatment)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
...
ggsave("output/4of.png", observed_features, height = 3, width = 3)

kruskal.test(observed_features ~ donor, data=meta)
observed_features <- ggplot(meta, aes(donor, observed_features)) + 
  geom_boxplot(aes(color = donor)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  theme(legend.title = element_blank()) +
  labs(y="Observed features", x = "") 
ggsave("output/of2.png", observed_features, height = 3, width = 3)

kruskal.test(shannon_entropy ~ treatment, data=meta)
shannon_entropy <- ggplot(meta, aes(treatment, shannon_entropy)) + 
  geom_boxplot(aes(color = treatment)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
...
ggsave("output/4se.png", shannon_entropy, height = 3, width = 3)

kruskal.test(shannon_entropy ~ donor, data=meta)
shannon_entropy <- ggplot(meta, aes(donor, shannon_entropy)) + 
  geom_boxplot(aes(color = donor)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  theme(legend.title = element_blank()) +
  labs(y="Shannon diversity", x = "") 
ggsave("output/se2.png", shannon_entropy, height = 3, width = 3)
