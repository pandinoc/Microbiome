...
treatment<-as.vector(unique(dataset$treatment))
datasetn<-dataset
...
for(i in 1:length(treatment)){
  ...
  print(paste("reading ",treatment[i],sep=""))
  temp<-subset(dataset, treatment==treatment[i])
  tempn<-subset(datasetn, get(my_column)==treatment[i])
  print(paste("finished reading ",treatment[i],sep=""))
...
treatment_ASVs<-subset(dataset, get(my_column)==treatment[1])

colnames(treatment_ASVs[1:10])
head(final_results)
ggplot(treatment_ASVs, aes(x = ASV15, y = ASV5)) +
  geom_point()
...
taxonomy<-read_qza("taxonomyS.qza")
head(taxonomy$data)
...
write.csv(subset(strong_results_taxa, trt=="RMD"), "output/moving-pictures-strong-results-taxa-RMD.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="WB"), "output/moving-pictures-strong-results-taxa-WB.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="ChIn"), "output/moving-pictures-strong-results-taxa-ChIn.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="HMP"), "output/moving-pictures-strong-results-taxa-HMP.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="blank"), "output/moving-pictures-strong-results-taxa-blank.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="donor"), "output/moving-pictures-strong-results-taxa-donor.csv", row.names = F)
